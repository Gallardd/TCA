...
function VotingStreamersSection({ onVotesSubmit, goBack }) {
    ...
    // Persistencia de categoría y votos
    const [currentCategoryIndex, setCurrentCategoryIndex] = useState(() => {
        const savedIndex = localStorage.getItem('streamersCategoryIndex');
        return savedIndex ? parseInt(savedIndex, 10) : 0;
    });
    const [votes, setVotes] = useState(() => {
        const savedVotes = localStorage.getItem('streamersVotes');
        return savedVotes
    ...
    useEffect(() => {
        localStorage.setItem('streamersCategoryIndex', currentCategoryIndex);
    }, [currentCategoryIndex]);

    useEffect(() => {
        localStorage.setItem('streamersVotes', JSON.stringify(votes));
    }, [votes]);
    ...
    const handleNextCategory = () => {
        ...
        if (currentCategoryIndex < categories.length - 1) {
            ...
            localStorage.removeItem('streamersCategoryIndex');
            localStorage.removeItem('streamersVotes');
            onVotesSubmit(votes);
        }
    };
